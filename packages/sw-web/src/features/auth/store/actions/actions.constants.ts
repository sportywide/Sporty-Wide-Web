export const LOGOUT = Symbol('LOGOUT');
export const LOGOUT_SUCCESS = Symbol('LOGOUT_SUCCESS');

export const LOGIN = Symbol('LOGIN');
export const LOGIN_SUCCESS = Symbol('LOGIN_SUCCESS');

export const SIGNUP = Symbol('SIGNUP');
export const SIGNUP_SUCCESS = Symbol('SIGNUP_SUCCESS');

export const RESEND_VERIFICATION_EMAIL = Symbol('RESEND_VERIFICATION_EMAIL');
export const RESEND_VERIFICATION_EMAIL_SUCCESS = Symbol('RESEND_VERIFICATION_EMAIL_SUCCESS');
